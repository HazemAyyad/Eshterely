...
@section('title', __('admin.featured_stores'))

@section('content')
<h4 class="py-4 mb-4">{{ __('admin.featured_stores') }}</h4>
...
<div class="card border-0 shadow-sm">
    <div class="card-header d-flex justify-content-between align-items-center">
        ...
        <a href="{{ route('admin.config.featured-stores.create') }}" class="btn btn-primary">
    ...
    <div class="table-responsive text-nowrap">
        <table id="featured-stores-table" class="table table-hover table-striped">
            <thead>
                <tr>
                    <th>#</th>
                    <th>store_slug</th>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const table = $('#featured-stores-table').DataTable({
        processing: true,
        serverSide: true,
        ajax: "{{ route('admin.config.featured-stores.data') }}",
        columns: [
            { data: 'id', name: 'id' },
    ...
    $(document).on('click', '#featured-stores-table .btn-delete', function() {
        const url = $(this).data('url');
        Swal.fire({ title: @json(__('admin.confirm_delete_store')), icon: 'warning', showCancelButton: true, confirmButtonText: @json(__('admin.yes')), cancelButtonText: @json(__('admin.no')) }).then((r) => {
